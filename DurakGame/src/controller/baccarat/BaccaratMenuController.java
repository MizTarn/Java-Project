...
public class BaccaratMenuController {
...
//		btnNext.visibleProperty().bind(actionHapping);
	...
	void createRoom(ActionEvent event) {
		...
		sending = "create_session_baccarat";
	}

	@FXML
	void handleNext(ActionEvent event) {
		if (Objects.equals(sending, "join_session_baccarat")) {
			...
			if (nameText == null || idText == null) {
				...
				try {
					GameApplication.client.joinBaccaratSession(nameText, idText, "base");
				} catch (NumberFormatException ex) {
					System.out.println("error");
				}

			}
		} else if (Objects.equals(sending, "create_session_baccarat")) {
			nameText = fieldName.getText();
			...
			if (nameText == null) {
				...
				try {
					GameApplication.client.createBaccaratSession(nameText, idText, "base");
				} catch (NumberFormatException ex) {
					System.out.println("error");
				}
			}
	...
	void handleNext1(ActionEvent event) {
		if (Objects.equals(sending, "join_session_baccarat")) {
			...
			if (nameText == null || idText == null) {
				...
				try {
					GameApplication.client.joinBaccaratSession(nameText, idText, "gui");
				} catch (NumberFormatException ex) {
					System.out.println("error");
				}
			...
			if (nameText == null) {
				...
				try {
					GameApplication.client.createBaccaratSession(nameText, idText, "gui");
				} catch (NumberFormatException ex) {
					System.out.println("error");
				}
			}
	...
	public void goBack(ActionEvent e) throws IOException {
		SceneLoader.loadScene(e, "/resource/fxml/ChoosePlayersBaccarat.fxml");
	}
